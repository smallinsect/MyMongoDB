...
if [ ! -d "/data/mongodb/mdboplog/mongo-$port" ];then
    mkdir -p /data/mongodb/mdboplog/mongo-$port
fi

if [ ! -d "/data/mongodb/mdboplog/log-$port" ];then
    mkdir -p /data/mongodb/mdboplog/log-$port
fi

bkdatapath=/data/mongodb/mdboplog/mongo-$port
bklogpath=/data/mongodb/mdboplog/log-$port
...
command_line="${command_linebin} ${host}:${port}"
...
/usr/bin/mongodump -h $host --port $port  -d local -c oplog.rs  --query '{ts:{$gte:Timestamp('$paramBakStartDate',1),$lte:Timestamp('$paramBakEndDate',9999)}}' -o $bkdatapath/mongodboplog$bkfilename
